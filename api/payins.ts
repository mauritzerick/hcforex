...
type VercelRequest = IncomingMessage & { body?: unknown; method?: string };
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  try {
    ...
    if (!currency || !['IDR', 'VND', 'KRW', 'JPY'].includes(currency) || typeof amount !== 'number') {
      res.status(400).json({
        error: 'Invalid body: currency (IDR|VND|KRW|JPY) and amount (number) required',
      });
      return;
    }
    ...
    const response = await fetch(`${BASE_URL}/payins`, {
      method: 'POST',
      headers: {
